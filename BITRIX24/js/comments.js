...
window.submitComment = async function() {
  ...
  try {
    const res = await fetch('api/comments.php', {
      ...
      body: JSON.stringify({
        section_id: window.currentCommentSectionId,
        text: text,
        user_name: window.bitrixUser ? 
          [window.bitrixUser.NAME, window.bitrixUser.LAST_NAME].filter(Boolean).join(' ') || 'Пользователь' : 
          'Пользователь'
      })
    });
    ...
    renderComment({
      user: window.bitrixUser ? 
          [window.bitrixUser.NAME, window.bitrixUser.LAST_NAME].filter(Boolean).join(' ') || 'Вы (Mock)' : 
          'Вы (Mock)',
